...
struct EmojiMemoryGameView: View {
    ...
    var body: some View {
        Group {
            if game.cards.allSatisfy({ $0.isMatched }) {
                VStack {
                    Text("You won 🎉")
                    Text("Score: \(Int(ceil(game.score)))")
                }
                .font(.largeTitle)
                ...
                .padding()
                .onAppear {
                    if !(game.cards.allSatisfy { !$0.isFaceUp && !$0.isMatched })
                    {
    ...
    private func dealAnimation(for card: EmojiMemoryGame.Card) -> Animation {
        ...
        return Animation.easeInOut(duration: CardConstants.dealDuration).delay(
            delay
        )
    }
    ...
    var gameBody: some View {
        ...
        .foregroundColor(Color(memoryColor: game.theme.color))
    }
                ...
                Group {
                    ...
                    if card.isConsumingBonusTime {
                        ...
                        .onAppear {
                            ...
                            withAnimation(
                                .linear(duration: card.bonusTimeRemaining)
                            ) {
